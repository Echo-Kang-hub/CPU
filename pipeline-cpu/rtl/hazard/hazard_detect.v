...
module hazard_detect(
    ...
    wire load_use_stall;
    wire alu_branch_jalr_stall;
    wire load_branch_jalr_stall;
    wire alu_csr_stall;
    ...
    assign alu_branch_jalr_stall = IFID_is_branch_jalr && IDEX_RegWrite && (IDEX_rd != 5'b0) && ((IDEX_rd == IFID_rs1) || (IDEX_rd == IFID_rs2));
    // load-branch: MA=load,ID=branch stall 1T，对于EX=load，ID=use，需阻塞2T，前1T阻塞由alu-branch/jalr实现
    assign load_branch_jalr_stall = IFID_is_branch_jalr && EXMA_MemRead && (EXMA_rd != 5'b0) && ((EXMA_rd == IFID_rs1) || (EXMA_rd == IFID_rs2));
    // alu-csr: EX=alu,ID=csr stall 1T
    assign alu_csr_stall = IFID_is_csr && IDEX_RegWrite && (IDEX_rd != 5'b0) && (IDEX_rd == IFID_rs1);

    assign stall = load_use_stall || alu_branch_jalr_stall || load_branch_jalr_stall || alu_csr_stall;
